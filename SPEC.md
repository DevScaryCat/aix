...
### 5b. 린트(경고) — 비차단 의도 점검 (`src/lint.mjs`)

검증기(`verify`)는 **"런타임이 스펙대로 정확히 실행되는가"** 를 전체·결정론적으로 판정한다(통과 = 실행 보장). 린트(`lint`)는 **다른·약한 질문** — **"스펙이 의도한 바와 일치하는가"** — 에 답한다. 타입검사로는 틀렸다고 증명할 수 없지만 대개 버그인 패턴이다.

**불변식:** 린트는 **절대 차단하지 않는다.** `verify`의 `errors[]`와 합/불 게이트를 건드리지 않는 **별도 채널**(`warnings[]`)로 반환되므로 "통과 = 실행 보장"의 전체성은 그대로다. 각 린트도 동일한 **닫힌 심볼 테이블** 위 유한 술어다(런타임 값·휴리스틱 없음).

| 코드 | 잡는 것 |
|---|---|
| `OPEN_MUTATION` | `auth` 없는 `update`/`delete` — 익명 클라이언트가 아무 행이나 수정·삭제 가능 (`create`+소유자는 이미 하드 오류 `OWNER_CREATE_NO_AUTH`로 막힘) |
| `OWNED_READ_OPEN` | 소유자 ref가 있는 엔티티를 `auth` 하에 노출하는데 읽기가 소유자로 한정 안 됨(`list`≠`list:mine`, `get`인데 `private` 없음) — 로그인한 누구나 **남의 행**을 봄. **단, 공개 카탈로그는 평범한 `list`가 정상**이라 의도 의존적 → *조언(advisory)* 으로만, 오류 아님 |

`aix check` 출력의 `warnings`에 실리고, `ok`나 종료 코드는 바뀌지 않는다.
